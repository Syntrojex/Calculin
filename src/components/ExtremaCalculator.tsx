...
import { solveExtrema1D, solveExtrema2D } from "@/lib/math-solver";
...
type Mode = "extrema1d" | "extrema2d";

export function ExtremaCalculator() {
  const settings = useSettings();
  const [mode, setMode] = useState<Mode>("extrema1d");
  ...
  useAutoRun([mode, expr, varX, varY, xMin, xMax, settings.numberForm, settings.decimalPlaces], solve, settings.autoCalculate);
        ...
        >
          <Tabs value={mode} onValueChange={(v) => setMode(v as Mode)}>
            <TabsList className="w-full">
              <TabsTrigger value="extrema1d" className="flex-1">f(x)</TabsTrigger>
              <TabsTrigger value="extrema2d" className="flex-1">f(x, y)</TabsTrigger>
            </TabsList>
          </Tabs>

          <div className="space-y-2">
            <Label>{mode === "extrema1d" ? "Function f(x)" : "Function f(x, y)"}</Label>
            <MathInput
              value={expr}
              onChange={setExpr}
